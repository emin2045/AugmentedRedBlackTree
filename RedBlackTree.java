...
public class RedBlackTree<I> {
	...
	private class Node { // private >> public
	...
	// private final Node nil = new Node(-1, (I)0, ""); // -1 in kaynaðý burasý
	...
	void tarihGonderen(Node x, I dateInput2) {
		if (x == nil) {
			return;
		} else {
			tarihGonderen(x.left, dateInput2);
			if (x.id.equals(dateInput2)) {
				// System.out.println(x.date);

				System.out.println("--------->\t" + getNumSmaller1(root, splitter(x.date)));
			}
			tarihGonderen(x.right, dateInput2);
	...
	void findDate(Node x, I id) { // find date with ID
		if (x == nil) {
			return;
		}
	...
	public void reading() throws FileNotFoundException {
		Scanner file = new Scanner(new File("C:\\Users\\asus\\Desktop\\ornek.txt"));
		String line;
		String[] parse = null;
		String[] date = null;
		int age = 0;
